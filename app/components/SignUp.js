...
class SignUp extends React.Component {
    ...
    handleSend() {
        let that = this;
       	if(this.state.secondsElapsed != '发送验证码') {
       		return;
       	}
        ...
        if (!this.regexMobile.test(mobile)) {
        	...
        	this.interval = setInterval(() => {
	            ...
	            if (secondsElapsed == '发送验证码') {
	                that.setState({ secondsElapsed: 60 });
	            } else {
	                that.setState({ secondsElapsed: secondsElapsed - 1 });
	            }
	            if (secondsElapsed == 0) {
	                clearInterval(that.interval);
	                that.setState({ secondsElapsed: '发送验证码' })
	            }
	        }, 1000);
    ...
    render() {
        ...
        return (
            <div className="si-wrap">
				<DocumentTitle title="报名信息"></DocumentTitle>
				...
				<ul className="list-group input-list-group">
					...
					<li className="list-group-item">
						...
						<button className={ 'btn btn-default btn-sendcode ' + (this.state.secondsElapsed == '发送验证码' ? '' : 'disabled')} type="button" onClick={this.handleSend.bind(this)}>{this.state.secondsElapsed}</button>
					</li>
				</ul>
