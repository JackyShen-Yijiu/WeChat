...
class CollectYcode extends React.Component {
    ...
    componentDidMount() {
        ...
        CollectYcodeActions.getUserInfo(this.props.params);
    }
    	...
    	return (
    		<div className="cy-wrap">
				...
				<div className="title">
					...
					<p>好友领取并成功报名</p>
					<p>双方将各得<span>巨额</span>返现</p>
					<div className="head-img">
						<img src="/img/weixin/222.jpg" className="img-circle" />
					</div>
					<p className="name">{user.name}</p>
				</div>
				<div className="y-code">
					<p className="code">Y码 <span>{ycode}</span></p>
				</div>

				<div className="btn-sign" style={this.state.isSave ? {display: 'block'} : {display: 'none'}}>
				...
				<div className="btn-sign other-btn" style={this.state.isSave ? {display: 'none'} : {display: 'block'}}>
					<p className="code-content"> Y码券已放入账户 <span>{this.state.mobile}</span></p>
					<button type="button" className="btn btn-default btn-lg btn-block">立即微信报名返现</button>
					<button type="button" className="btn btn-info btn-lg btn-block">下载APP报名返现</button>
				</div>
