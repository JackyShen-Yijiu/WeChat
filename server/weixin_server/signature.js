...
var rediscache=require("../common/cache");
var cache = {
    ticket: null,
    time: 0
};
...
function getSignature(config, url, cb) {
    ...
    if (!cache || !cache.ticket) {
        ...
        rediscache.get("ticket"+config.id,function(err,data){
            ...
            request.get(tokenUrl, function(error, response, body) {
                if (error) {
                    ...
                    try {
                        ...
                        rediscache.set("accessToken"+config.id,token,60*100,function(){});
                        cb(null, token);
                    }
                    catch (e) {
                        cb('getToken error', e);
    ...
    if (!cache.ticket || (new Date().getTime() - cache.time) > 7000000) {
        async.waterfall([function(cb) {
            ...
            if (error) {
                ...
                // 文件保存
               rediscache.set("ticket"+config.id,cache,60*1000,function(err,data){});
